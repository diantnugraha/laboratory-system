...
export default function ContractNewPage() {
  ...
  // Fetch auto-generated contract code
  const fetchContractCode = useCallback(async () => {
    try {
      const response = await contractService.generateCode();
  ...
  useEffect(() => {
    ...
    fetchServices();
    fetchPackages();
  }, [fetchContractCode, fetchServices, fetchPackages]);
  ...
  const onSubmit = async (data: ContractFormData) => {
    try {
      ...
      const payload = {
        ...
        contract_document: contractDocument?.name || undefined,
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pb-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-foreground">Add New Contract</h1>
              <p className="text-sm text-muted-foreground mt-1">Create a new contract record</p>
            </div>
          </div>
                    ...
                    <FormControl>
                      <div className="relative">
                        <Input
                          type="date"
                          {...field}
                          className="h-10 bg-muted/30 hover:bg-muted/50 focus:bg-background transition-colors border-muted pl-3 pr-10 [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:right-0 [&::-webkit-calendar-picker-indicator]:w-10 [&::-webkit-calendar-picker-indicator]:h-full [&::-webkit-calendar-picker-indicator]:cursor-pointer"
                        />
