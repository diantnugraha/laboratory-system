...
const columns = [
  ...
  { 
    key: "description", 
    label: "Description",
    render: (item: Unit) => <RenderHTML html={item.description} />,
  },
  { 
    key: "lab", 
    label: "Laboratory",
...
export default function UnitsPage() {
  ...
  const fetchUnits = useCallback(async (page: number = 1, search?: string) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    fetchUnits(pagination.page, debouncedSearch);
  }, [fetchUnits, pagination.page, debouncedSearch]);
                  ...
                  <TableRow key={item.id}>
                    {columns.map((column) => (
                      <TableCell key={String(column.key)}>
                        {column.render
                          ? column.render(item)
                          : String(item[column.key as keyof Unit] ?? "")}
                      </TableCell>
                    ))}
                  </TableRow>
                ))
