...
const columns = [
  ...
  { 
    ...
    label: "Parameter",
    render: (item: Service) => (
      <RenderHTML html={item.parameter?.name || "N/A"} />
    ),
  },
  { 
    key: "method", 
    label: "Method",
    render: (item: Service) => (
      <RenderHTML html={item.method?.name || "N/A"} />
    ),
  },
  {
    key: "status",
    ...
    render: (item: Service) => (
      <Badge variant={item.status === "Active" ? "default" : "secondary"}>
        <RenderHTML html={item.status || "N/A"} />
      </Badge>
    ),
  },
  {
    key: "price",
    label: "Price (IDR)",
    render: (item: Service) => formatCurrency(item.price),
  },
];
    ...
    <>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-foreground">Service</h1>
                  ...
                  <TableRow key={item.id}>
                    {columns.map((column) => (
                      <TableCell key={String(column.key)}>
                        {column.render
                          ? column.render(item)
                          : String(item[column.key as keyof Service] ?? "")}
                      </TableCell>
                    ))}
                  </TableRow>
                ))
