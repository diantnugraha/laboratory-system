...
import { CategoryServiceFormDialog } from "@/components/forms/CategoryServiceFormDialog";
...
const columns = [
  { 
    key: "name", 
    label: "Name Categories Service",
    render: (item: Category) => (
      <Link 
        href={`/master/categories-services/${item.id}`}
    ...
    <>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-foreground">Categories Services</h1>
                  ...
                  <TableRow key={item.id}>
                    {columns.map((column) => (
                      <TableCell key={String(column.key)}>
                        {column.render
                          ? column.render(item)
                          : String(item[column.key as keyof Category] ?? "")}
                      </TableCell>
                    ))}
                  </TableRow>
                ))
