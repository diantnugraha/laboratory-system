    ...
    const fetchPreorder = async () => {
      try {
        ...
        if (data.customer) {
          ...
          // Fetch customer details for contacts
          try {
            const customerResponse = await customerService.getById(data.customer.id);
            if (customerResponse.data) {
              setContacts(customerResponse.data.contacts || []);
              setSelectedCustomer({
                ...customer,
        ...
        form.reset({
          ...
          submitedBy: data.submited_by || "",
          receivedDate: data.received_date ? parseISO(data.received_date) : undefined,
          delivery: data.delivery || "",
          priority: data.priority || "normal",
          lab: data.lab || 1,
          sampleQuantity: data.sample_quantity || 1,
          coveringLetter: data.covering_letter === "1" || data.covering_letter === "true",
          testingParameters: data.testing_parameters === "1" || data.testing_parameters === "true",
          allSampleSubcontracted: data.subcon === 1,
          subconId: data.subcon_id || undefined,
          subconDue: data.subcon_due ? parseISO(data.subcon_due) : undefined,
          remarks: data.remarks || "",
          notesCustomer: data.notes_customer || "",
        });
      } catch (error) {
